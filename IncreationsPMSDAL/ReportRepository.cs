   ...
   public  class ReportRepository : BaseRepository 
    {
        ...
        public List<PendingPaymentSchedule> GetPendingPaymentScheduleReport(string Client = "")
        {
            using (IDbConnection connection = OpenConnection(dataConnection))
            {
                string query = @"

                   select ClientName,ProjectEnquiry,ProjectPaymentSchedule.date,Description,Amount 
                   from Project
                   inner join ProjectPaymentSchedule on ProjectPaymentSchedule.ProjectId =Project.ProjectId 
                   inner join Client on Client.ClientId =Project.ClientId
                   where Paymentid not in (select PaymentScheduleid  from CustomerInvoiceItem)
                   and ClientName LIKE '%'+@Client+'%' 
                   order by ProjectPaymentSchedule.date";

                return connection.Query<PendingPaymentSchedule>(query, new { Client = Client }).ToList();
            }
        }
    }
}
